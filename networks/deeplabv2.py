...
class ResNet101(nn.Module):
    ...
    def forward(self, input, lbl=None, class_weight=None): # input : B3HW, lbl : BHW
        x = self.conv1(input)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        pred_feat = self.layer5(x) # last prediction layer

        _, ht, wt = lbl.size()
        pred_feat = F.interpolate(pred_feat, (ht,wt), mode='bilinear', align_corners=True)

        if self.training:
            if lbl is not None:
                self.loss_CE = self.CrossEntropy2d(pred_feat, lbl, class_weight=class_weight)
            else :
                assert 0
